...
CREATE VIEW [dbo].[UCP_PersonsV2]
AS
SELECT DISTINCT 
                         TOP (1000000) t1.NAME AS FullName, COALESCE (t1.EMP_ID, t1.PPS_ID) AS EmployeeID, t1.FIRST_NAME AS FirstName, t1.MIDDLE_NAME AS MiddleName, t1.LAST_NAME AS LastName, t1.NAME_SUFFIX AS Suffix, 
                         t1.BIRTHDATE, t1.EMAIL AS UCDMailID, t1.UCD_LOGIN_ID AS UCDLoginID, t1.JOB_DEPT AS HomeDepartment, CASE WHEN JOB_DEPT = HomeDept THEN AltDept1 ELSE AltDept2 END AS AlternateDepartment, 
...
WHERE        (t1.JOB_IND = 'P') AND EFF_DT
	= (
		SELECT MAX(EFF_DT)
		FROM dbo.UCPath_PersonJob	 t2
		WHERE t1.EMP_ID = t2.EMP_ID
		AND t2.EFF_DT <= GETDATE()
		AND t1.JOB_IND = t2.JOB_IND
	)
ORDER BY FullName
GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 1, @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'UCP_PersonsV2';
...
Begin DesignProperties = 
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'UCP_PersonsV2';
