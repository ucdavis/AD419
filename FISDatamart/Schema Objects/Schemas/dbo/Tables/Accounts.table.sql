CREATE TABLE [dbo].[Accounts] (
    ...
    [PrincipalInvestigatorName] VARCHAR (50)    NULL,
...
GO
CREATE NONCLUSTERED INDEX [Accounts_YearPeriodChartOrg_Account_CVIDX]
    ON [dbo].[Accounts]([Year] ASC, [Period] ASC, [Chart] ASC, [Org] ASC)
    INCLUDE([Account]);


GO
CREATE NONCLUSTERED INDEX [Accounts_FundGroupCode_CVIDX_2]
    ON [dbo].[Accounts]([FundGroupCode] ASC)
    INCLUDE([Org], [AccountName], [SubFundGroupNum], [SubFundGroupTypeCode], [MgrName], [FunctionCodeID], [OPFundFK], [AnnualReportCode], [FringeBenefitIndicator], [FringeBenefitChart], [FringeBenefitAccount], [AccountPK], [OrgFK], [AwardEndDate], [AwardAmount], [HigherEdFuncCode], [A11AcctNum], [OpFundNum], [OpFundGroupCode], [PrincipalInvestigatorName], [TypeCode], [Purpose], [FederalAgencyCode], [AwardNum], [AwardTypeCode]);
...
GO
CREATE NONCLUSTERED INDEX [Accounts_AccountPeriod_CVIDX]
    ON [dbo].[Accounts]([Period] ASC)
    INCLUDE([Year], [Chart], [Account], [Org], [AccountName], [FringeBenefitChart], [FringeBenefitAccount], [OrgFK], [FunctionCodeID], [OPFundFK], [HigherEdFuncCode], [A11AcctNum], [OpFundNum], [OpFundGroupCode], [AnnualReportCode], [FringeBenefitIndicator], [Purpose], [FederalAgencyCode], [AwardNum], [AwardTypeCode], [AwardEndDate], [AwardAmount], [SubFundGroupNum], [SubFundGroupTypeCode], [FundGroupCode], [MgrName], [PrincipalInvestigatorName], [TypeCode]);
