...
SELECT        CONVERT(CHAR(4), TransV.Year) 
                         + '|' + '00' + '|' + TransV.Chart + '|' + TransV.Account + '|' + TransV.SubAccount + '|' + TransV.ObjectType + '|' + TransV.Object + '|' + TransV.SubObject + '|' + TransV.BalType
                          + '|' + '' + '|' + '' + '|' + '' + '|' + '' + '|' + '' + '|' + CONVERT(varchar(20), TransV.BalanceCreateDate, 112) AS PKTrans, 
                         Organizations.OrganizationPK AS OrganizationFK, TransV.Year AS FiscalYear, '00' AS FiscalPeriod, TransV.Chart, dbo.Accounts.Org AS OrgCode, 
                         ...
                         Organizations.OrgR AS Level3_OrgCode, Organizations.NameR AS Level3_OrgName, TransV.Chart + '-' + TransV.Account AS Account, 
                         TransV.Account AS AccountNum, dbo.Accounts.AccountName, dbo.Accounts.MgrName AS AccountManager, 
                         ...
                         TransV.SubAccount, dbo.SubAccounts.SubAccountName, TransV.Object AS ObjectCode, dbo.Objects.Name AS ObjectName, 
                         ...
                         dbo.Projects.ManagerID AS ProjectManager, dbo.Projects.Description AS ProjectDescription, CONVERT(char(4), NULL) AS TransDocType, 
                         CONVERT(varchar(40), NULL) AS TransDocTypeName, CONVERT(char(2), NULL) AS TransDocOrigin, CONVERT(varchar(12), NULL) AS DocumentNumber, 
                         CONVERT(char(9), NULL) AS TransDocNum, CONVERT(char(10), NULL) AS TransDocTrackNum, CONVERT(char(8), NULL) AS TransDocInitiator, 
                         TransV.BalanceCreateDate AS TransInitDate, CONVERT(decimal(7, 0), NULL) AS LineSequenceNum, CONVERT(varchar(40), NULL) AS TransDescription, 
                         TransV.FiscalYearBeginningBalance AS TransLineAmount, TransV.BalType AS TransBalanceType, 
                         CASE WHEN TransV.BalType = 'AC' THEN TransV.FiscalYearBeginningBalance ELSE 0 END AS ExpendAmount, 
