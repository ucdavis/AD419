...
SELECT @Statement = 'EXEC FISDataMart.dbo.usp_GetOrgLevel @OrgId =' + '''' + @OrgId + ''', @OrgLevel = @OrgLevel OUTPUT' 
		...
		SELECT 
			...
			FROM ( 
				...
				WHERE	
					...
					(A.Account NOT IN (SELECT Account FROM FISDataMart.dbo.CentralAccounts WHERE OrgId = ''AAES'')) AND
