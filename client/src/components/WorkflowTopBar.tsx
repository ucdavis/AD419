...
export function WorkflowTopBar({ snapshot }: { snapshot: WorkflowSnapshot }) {
  const user = useUser();
  ...
  return (
    <header className="workflow-topbar">
      ...
      <div className="workflow-topbar__context">
        <span>Cycle {snapshot.cycle.label}</span>
        <span>{snapshot.organization}</span>
      </div>
      <nav aria-label="Application actions" className="workflow-topbar__actions">
        <button className="btn btn-ghost btn-sm text-white">Activity log</button>
        <button className="btn btn-ghost btn-sm text-white">Help</button>
        <div
