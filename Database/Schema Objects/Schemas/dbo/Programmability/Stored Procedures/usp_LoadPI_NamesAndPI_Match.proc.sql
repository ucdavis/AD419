...
BEGIN
	...
	IF @IsDebug = 1
		BEGIN
			...
			select OrgR, Employee_Name, EID, PI_LFM PI 
			from @PI_Names
			...
			select 
				...
				Accession, 
				UPPER(REPLACE (PI, '.', '')) PI, 
				CASE WHEN a.EID IS NULL 
					THEN b.EID
					ELSE a.EID 
				END EID, 
				CASE WHEN a.PI_LFM IS NULL THEN  b.PI_LF
					ELSE a.PI_LFM 
				END PI_Match
			from @Table t1
			LEFT OUTER JOIN @PI_Names a ON RTRIM(UPPER(REPLACE (PI, '.', '')))   = RTRIM(a.PI_LFM)
			LEFT OUTER JOIN @PI_Names b ON RTRIM(UPPER(REPLACE (PI, '.', '')))  = RTRIM(b.PI_LF)
			WHERE CASE WHEN a.PI_LFM IS NULL THEN  b.PI_LF
					ELSE a.PI_LFM 
				END IS NOT NULL
			group by  RTRIM(UPPER(REPLACE (PI, '.', ''))) ,  PI, b.PI_LF, b.PI_LFM, a.PI_LFM, a.PI_LF, accession, b.EID, a.EID, t1.OrgR
