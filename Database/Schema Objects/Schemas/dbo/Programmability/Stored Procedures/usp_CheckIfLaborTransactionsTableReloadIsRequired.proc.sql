...
--
-- This check is made because the LaborTransactions table load process takes some time and only needs to be done once
...
--
-- =============================================
CREATE PROCEDURE usp_CheckIfLaborTransactionsTableReloadIsRequired
(
	@FiscalYear int = 2013, --The ending year of the current AD-419 Reporting Year
...
DECLARE @StartingPayPeriodYear int = ISNULL((SELECT YEAR(MIN([PayPeriodEndDate])) FROM [AD419].[dbo].[LaborTransactions]), 0)
DECLARE @EndingPayPeriodYear int = ISNULL((SELECT YEAR(MAX([PayPeriodEndDate])) FROM [AD419].[dbo].[LaborTransactions]), 0)
...
IF @StartingFiscalYear != @StartingPayPeriodYear OR  @EndingFiscalYear != @EndingPayPeriodYear
	BEGIN
		--IF @IsDebug = 1
			PRINT 'Starting Fiscal Year (' + CONVERT(varchar(4), @StartingFiscalYear) + ') does NOT match Starting Pay Period Fiscal Year (' + CONVERT(varchar(4), @StartingPayPeriodYear) + ') OR 
Ending Fiscal Year (' + CONVERT(varchar(4), @EndingFiscalYear) + ' does NOT match Ending Pay Period Fiscal Year (' + CONVERT(varchar(4), @EndingPayPeriodYear) + '); therefore, reloading LaborTransactions table
for ' + CONVERT(varchar(4), @StartingFiscalYear) + '-' + CONVERT(varchar(4), @EndingFiscalYear) + ' AD-419 Reporting Year.

'

--		SELECT @TSQL = 'EXEC @return_value = dbo.usp_LoadLaborTransactions @FiscalYear = ' + CONVERT(varchar(4), @FiscalYear) + ', @IsDebug = ' + CONVERT(varchar(1), @IsDebug) + '
--SELECT	''Return Value'' = @return_value
...
--		SELECT @TSQL = 'EXEC @return_value = usp_UpdateLaborTransactionsMissingEmployeeNames @IsDebug = ' + CONVERT(varchar(1), @IsDebug) + '
--SELECT	''Return Value'' = @return_value
...
ELSE
	BEGIN
		--IF @IsDebug = 1
			PRINT 'Starting Fiscal Year (' + CONVERT(varchar(4), @StartingFiscalYear) + ') matches Starting Pay Period Fiscal Year (' + CONVERT(varchar(4), @StartingPayPeriodYear) + ') AND 
Ending Fiscal Year (' + CONVERT(varchar(4), @EndingFiscalYear) + ') matches Ending Pay Period Fiscal Year (' + CONVERT(varchar(4), @EndingPayPeriodYear) + '); therefore, BYPASSING reloading LaborTransactions table
for ' + CONVERT(varchar(4), @StartingFiscalYear) + '-' + CONVERT(varchar(4), @EndingFiscalYear) + ' AD-419 Reporting Year.'
