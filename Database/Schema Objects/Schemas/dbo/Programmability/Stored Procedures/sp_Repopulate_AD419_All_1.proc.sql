...
-- 2015-03-31 by kjt: Reinstated SFN 204 logic as we'll need this for the 2015 Reporting Year.
CREATE PROCEDURE sp_Repopulate_AD419_All 
( 
...
-- This step replaces the above and populates the Expenses_CAES with FFY expenses
-- for use in gathering the SFN 201-205 expenses only.
EXEC @return_value = sp_RePopulate_FFY_CAES_Expenses_Using_ARC
	 @FiscalYear = @FiscalYear,
	 @IsDebug = @IsDebug
SELECT	'Return Value' = @return_value
...
-- This step is no longer used as of the 2015 Reporting Year since we will be 
-- reporting on SFN 204 expenses going forward.
...
-- These steps are needed once again as we will be reporting on SFN 204 expenses starting the 2015 reporting year.
...
-- 2013-2014:  The following modifications were needed to be made to the FFY_2014_SFN_ENTRIES table,
-- because we ended up with multiple accession numbers for the same project numbers:
-- Update the accession numbers for accounts that accrued expenses under the duplicate, expired projects:

  UPDATE FFY_2014_SFN_ENTRIES
  SET  Accession = '0229489' WHERE Account = 'AEN5J10' AND IsActive = 0 AND Accession Is Null
   UPDATE FFY_2014_SFN_ENTRIES
  SET  Accession = '0230718' WHERE Account = 'APP5LE6' AND IsActive = 0 AND Accession Is Null

    UPDATE FFY_2014_SFN_ENTRIES
  SET  Accession = '1002199' WHERE Account = 'ADO1106' AND IsActive = 0 AND Accession Is Null

--Get rid of the credit for the one expired project:
  DELETE FROM FFY_2014_SFN_ENTRIES WHERE AwardNum = 'CA-D*-NPB-4893-H'

-- Lastly, re-run the sp_INSERT_20x_EXPENSE_SUMS_INTO_EXPENSES procedure now that the 3 duplicated have been disabled:
	EXEC	@return_value = [dbo].[sp_INSERT_20x_EXPENSE_SUMS_INTO_EXPENSES]
		@FiscalYear = @FiscalYear
	SELECT	'Return Value' = @return_value
...
-- New for 2014:
-- Run the procedures that automatically associate the 241 employee expenses so that the accounting team
-- does not have to do it manually:

-- Step 1: Populate the PI_Names and PI_Match tables
	EXEC	@return_value = [dbo].[usp_LoadPI_NamesAndPI_Match]
		@IsDebug = @IsDebug
	SELECT	'Return Value' = @return_value

-- Step 2: Populate the PI_Name_Exceptions for any employees that didn't get matched in step 1:
	EXEC	@return_value = [dbo].[usp_LoadPI_Name_Exceptions]
		@IsDebug = @IsDebug
	SELECT	'Return Value' = @return_value

-- Step 3: Make any updates to the PI_Match table using the modified employee names from the PI_Name_Exceptions table:
	EXEC	@return_value = [dbo].[usp_UpdatePI_MatchUsingPI_NameExceptions]
		@IsDebug = @IsDebug
	SELECT	'Return Value' = @return_value

-- Step  4: Insert the associations for the 241 PI and 241 Non-PI expenses using the PI_Match and PI_Names tables:
	EXEC	@return_value = [dbo].[usp_InsertAssociationsFor241Employees]
		@IsDebug = @IsDebug
	SELECT	'Return Value' = @return_value

/*
2015-03-31 by kjt: Starting the 2015 Reporting Year we will be reporting in reporting on SFN 204 expenses
once again; although, we won't be using the UI to make any 204 associations.  They'll need to run the report
located at , and let me know what the corresponding accession numbers are.
Once that is done we will run the SFN 204 Expense Accounts with Missing Project Numbers (Accession) at URL:
http://reports.caes.ucdavis.edu/Reports/Pages/Report.aspx?ItemPath=%2fAD419Reports%2fSFN+204+Accounts+with+Missing+Projects
after that you will need to manually update the 204AcctXProj table with the corresponding accession numbers
for any projects not automatically resolved.  Finally, rerun the sp_Repopulate_AD419_204_Expenses sproc.
Not sure you'll need to run the any other sprocs mentioned below.
