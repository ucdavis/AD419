...
-- Usage:
/*
	EXEC usp_BeginProcessingForNewReportingPeriod @FiscalYear = 2015, @IsDebug = 1
*/
--
-- Modifications:
...
BEGIN
	...
	SELECT ' + CONVERT(varchar(4), @FiscalYear) + ' [Year]
      ...
      ,[Is204]
      ,[AwardNumber]
      ,[ProjectNumber] FROM [dbo].[ArcCodeAccountExclusions]
	WHERE Year = ' + CONVERT(varchar(4), @FiscalYear -1) + ' 
	EXCEPT
	SELECT * FROM [dbo].[ArcCodeAccountExclusions]
	WHERE Year = ' + CONVERT(varchar(4), @FiscalYear) + '
