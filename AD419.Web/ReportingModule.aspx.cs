...
namespace CAESDO
{
    public partial class ReportingModule : ApplicationPage
    {
        ...
        {
            // Get or initialize the saved data array:
            IOrderedDictionary associatedProjectData;  // To hold any previously saved data retrieved from the viewstate.

            if (ViewState["AssociatedProjectData"] == null)
            {
                associatedProjectData = new OrderedDictionary();
            }
            else
            {
            ...
            foreach (GridViewRow row in gvAssociationProjects.Rows)
            {
                ...
                if (row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    // Remove row data that would otherwise be restored after a sort:
                    if (associatedProjectData.Contains(gvAssociationProjects.DataKeys[i].Value))
                        associatedProjectData.Remove(gvAssociationProjects.DataKeys[i].Value);
                    ...
                    if (cbox.Checked == true)
                    {
                        ...
                        // Save row data to be restored after a sort:
                        if (!associatedProjectData.Contains(gvAssociationProjects.DataKeys[i].Value))
                            associatedProjectData.Add(gvAssociationProjects.DataKeys[i].Value, populateValuesToSave(row));
                    }
                    else
                    {
        ...
        /// <summary>
        /// Created: March 2012 by Ken Taylor
        /// Purpose:
        /// This method restores "erased" gridview data after a sort has occured so that any data that
        /// was displayed prior to the sort is displayed after the sort, but just in the new sort order.
        ///
        /// Background:
        /// Steve Pesis had asked that I add PI to the gvAssociationProjects.  Once I did that they also
        /// wanted to have it sorted by PI as this is how most of the associations are made.
        /// Since it was formerly sorted by project, I thought it would be a nice addition to add a sort
        /// feature that allowed them to sort by PI or project.  That was where the fun begin.
        ///
        /// Scott had progammatically manipulated some of the fields in gvAssociationProjects after
        /// the project number and newly added PI field were initially databound.  This allowed users
        /// to select/deselect check boxes and alter percentages independently and prior to making the
        /// changes permanent.
        ///
        /// This meant that once I added sorting on the project and PI fields, any values that were populated
        /// after the initial databinding were being lost when a sort was executed.  This is because of the
        /// re-databinding that happens in order to accomplish the sort.
        ///
        /// Therefore, I modified the updateAssociations method to save the "after the databind" data in the
        /// viewstate and restore it once the sorting's re-databinding had occured.  I tried placing the
        /// data restoration logic in several more intuitive methods like gridview_OnSorting and gridview_OnSorted;
        /// however, my research indicated that the re-databinding happens after the gridview_OnSorted event
        /// has fired, so any changes made in that method will be lost as well.  Placing it here in this method is
        /// the only place I could get it to work successfully.  I also needed to add some logic to the
        /// clearGvAssociationProjects and cboxExpense_CheckedChanged methods to clear/reset the saved values
        /// in the ViewState, and the updateAssociations method to save, update or remove any changed values
        /// from view state's "to-be-restored" records.
        ///
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void gvAssociationProjects_DataBound(object sender, EventArgs e)
        {
            // Get or initialize the saved data array:
            IOrderedDictionary associatedProjectData;  // To hold any previously saved data retrieved from the viewstate.
        ...
        protected void lbtnNumCheckedProjects_Click(object sender, EventArgs e)
        {
            ...
            /*****************************************************************
             * 2012-03-19 by kjt: Added this logic block that will toggle
             * from select all to select none, where formerly it only would
             * select all.
             *****************************************************************/
            var isChecked = true;
            var lbtnNumCheckedProjects = (LinkButton)gvAssociationProjects.HeaderRow.FindControl("lbtnNumCheckedProjects");//The header cell of the checkbox column
            var numProjectsChecked = Convert.ToInt32(lbtnNumCheckedProjects.Text);

            if (numProjectsChecked == gvAssociationProjects.Rows.Count)
            {
                // This means that all projects were formerly checked,
                // so the user reclicked the box to uncheck all the
                // boxes.
                // Set isChecked to false to uncheck all the boxes:
                isChecked = false;
            }
            else
            {
                // This was relocated from its original location.
                //Set/reset the number checked to 0.
                ViewState["NumCheckedProjects"] = 0;
            }
            /*****************************************************************/
        ...
        /// <summary>
        /// Given a GridViewRow, get the values and insert them into a new
        /// OrderedDictionary so that they can be restored after a sort.
        /// </summary>
        /// <param name="row">The GridViewRow with the data to be saved.</param>
        /// <returns>An OrderedDictionary containing the row's data.</returns>
        private IOrderedDictionary populateValuesToSave(GridViewRow row)
        {
            var rowData = new OrderedDictionary();

            // In all cases the check bix will be checked, so it is not necessary to save it.

            var currentTextBox = (TextBox)row.FindControl("txtAssociatePercent");
            rowData.Add("AssociatePercent", currentTextBox.Text);

            var projectSpent = row.Cells[AD419Configuration.cellIndexProjectSpent].Text;
            rowData.Add("ProjectSpent", projectSpent);

            var projectsFte = row.Cells[AD419Configuration.cellIndexProjectsFTE].Text;
            rowData.Add("ProjectsFTE", projectsFte);
        ...
        private void updateAssociations(DataSet AssociationsData, double TotalSpent, bool addValues)
        {
            ...
            // Init OrderedDictionary for saving row data if necessary:
            if (ViewState["AssociatedProjectData"] == null)
                ViewState["AssociatedProjectData"] = new OrderedDictionary();

            // Retrieve the saved row data OrderedDictionary from the viewstate:
            var associatedProjectData = ViewState["AssociatedProjectData"] as IOrderedDictionary;
            ...
            if (AssociationsData.Tables[0].Rows.Count != 0)
            {
                ...
                foreach (GridViewRow row in gvAssociationProjects.Rows)
                {
                    ...
                    var accession = gvAssociationProjects.DataKeys[row.RowIndex].Value;
                    ...
                    if (TotalSpent == 0)
                    {
                        ...
                        if (currentCheckBox.Checked == true)
                        {
                            ...
                            // Remove row data that would otherwise be restored after a sort:
                            associatedProjectData.Remove(accession);
                        }
                    }
                    else
                    {
                        ...
                        if (CurrentAssociationsRow != null)
                        {
                            ...
                            if (row.Cells[AD419Configuration.cellIndexProjectSpent].Text == string.Empty)
                            {
                                ...
                                if (currentCheckBox.Checked == false)
                                {
                                    ...
                                    // Save row data to be restored after a sort:
                                    associatedProjectData.Add(accession, populateValuesToSave(row));
                                }
                            }
                            else
                            {
        ...
        private void clearGvAssociationProjects()
        {
            ...
            // Clear out any saved row data:
            ViewState["AssociatedProjectData"] = null;

            // Also set the header's num checked projects back to zero:
            gvAssociationProjects.HeaderRow.Cells[0].Text = "0";
